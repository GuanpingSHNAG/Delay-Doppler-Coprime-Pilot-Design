...
# ==========================================
# 5. 遍历参数，找全局最优；若其为非互质，则同时记录互质次优
# ==========================================
lambda_max_list = [3]
cond_num_list = [1, 2, 3, 5, 8, 10]
angle_deg_list = np.arange(0, 91, 5)
...
for lam_max in lambda_max_list:
    for cond_num in cond_num_list:
        ...
        for angle_deg in angle_deg_list:
            ...
            # 全局最优
            global_best_idx = np.argmax(det_arr)
            global_best_det = det_arr[global_best_idx]
            global_best_locs = valid_combos[global_best_idx]

            # 如果全局最优是非互质的 (gcd_flag==False)
            if not gcd_flag_arr[global_best_idx]:
                # 记录非互质最优（即全局最优）
                results.append({
                    'type': 'global_best_non_coprime',
                    'lambda_max': lam_max,
                    'cond_num': cond_num,
                    'angle_deg': angle_deg,
                    'locs': global_best_locs,
                    'det': global_best_det,
                })

                # 找出互质组合中的最优（次优，因为全局最优是非互质的）
                coprime_indices = np.where(gcd_flag_arr)[0]
                if len(coprime_indices) > 0:
                    best_coprime_idx = coprime_indices[np.argmax(det_arr[coprime_indices])]
                    best_coprime_det = det_arr[best_coprime_idx]
                    best_coprime_locs = valid_combos[best_coprime_idx]
                    results.append({
                        'type': 'coprime_suboptimal',
                        'lambda_max': lam_max,
                        'cond_num': cond_num,
                        'angle_deg': angle_deg,
                        'locs': best_coprime_locs,
                        'det': best_coprime_det,
                    })
                # else: 如果没有互质组合，则只记录非互质最优（这种情况极少见）
            # 否则（全局最优是互质的），不输出任何内容

# ==========================================
# 6. 输出结果
# ==========================================
print(f"\n遍历完成。共找到 {len(results)} 条记录（每个参数组合可能包含两条：非互质全局最优 + 互质次优）。")
if len(results) > 0:
    print("\n结果示例（前10条）：")
    print("=" * 100)
    for r in results[:10]:
        print(f"{r['type']}: λ_max={r['lambda_max']}, cond={r['cond_num']}, angle={r['angle_deg']}° -> 位置: {r['locs']} (det={r['det']:.4e})")

    # 保存到 CSV
    with open('optimal_non_coprime_and_coprime_suboptimal.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['type', 'lambda_max', 'cond_num', 'angle_deg', 'pilot_locations', 'det_value'])
        for r in results:
            locs_str = ';'.join([f"({m},{n})" for m, n in r['locs']])
            writer.writerow([r['type'], r['lambda_max'], r['cond_num'], r['angle_deg'], locs_str, r['det']])
    print("\n结果已保存至 optimal_non_coprime_and_coprime_suboptimal.csv")
else:
    print("没有发现任何满足条件的参数组合（所有全局最优均为互质模式）。")